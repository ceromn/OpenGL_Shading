...
{
public:
	deferred_shading_app() : render_program_nm(0),
		render_program(0),
		light_program(0),
		use_nm(true),
		paused(false),
...
void deferred_shading_app::init()
{
	static const char title[] = "Deferred Shading";
...
void deferred_shading_app::startup()
{
	...
	tex_nm = sb7::ktx::file::load("../extern/media/textures/ladybug_nm.ktx");
	tex_diffuse = sb7::ktx::file::load("../extern/media/textures/ladybug_co.ktx");
...
void deferred_shading_app::render(double currentTime)
{
...
void deferred_shading_app::shutdown()
{
...
void deferred_shading_app::load_shaders()
{
	GLuint shaders1[2];
	if (render_program)
		glDeleteProgram(render_program);
	shaders1[0] = sb7::shader::load("../extern/media/shaders/deferredshading/render.vs.glsl", GL_VERTEX_SHADER);
	shaders1[1] = sb7::shader::load("../extern/media/shaders/deferredshading/render.fs.glsl", GL_FRAGMENT_SHADER);
	render_program = sb7::program::link_from_shaders(shaders1, 2, true);
	...
	shaders2[0] = sb7::shader::load("../extern/media/shaders/deferredshading/render-nm.vs.glsl", GL_VERTEX_SHADER);
	shaders2[1] = sb7::shader::load("../extern/media/shaders/deferredshading/render-nm.fs.glsl", GL_FRAGMENT_SHADER);
	render_program_nm = sb7::program::link_from_shaders(shaders2, 2, true);
	...
	if (light_program)
		glDeleteProgram(light_program);
	shaders3[0] = sb7::shader::load("../extern/media/shaders/deferredshading/light.vs.glsl", GL_VERTEX_SHADER);
	shaders3[1] = sb7::shader::load("../extern/media/shaders/deferredshading/light.fs.glsl", GL_FRAGMENT_SHADER);
	light_program = sb7::program::link_from_shaders(shaders3, 2, true);
	...
	shaders4[0] = sb7::shader::load("../extern/media/shaders/deferredshading/light.vs.glsl", GL_VERTEX_SHADER);
	shaders4[1] = sb7::shader::load("../extern/media/shaders/deferredshading/render-vis.fs.glsl", GL_FRAGMENT_SHADER);
	vis_program = sb7::program::link_from_shaders(shaders4, 2, true);

}

void deferred_shading_app::onKey(int key, int action)
{
	if (action)
	{
		switch (key)
